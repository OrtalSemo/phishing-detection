...
dataset = load_dataset("csv", data_files="labeled_data/combined_balanced_data.csv", split="train")
...
def tokenize_function(example):
    return tokenizer(
        ...
        padding="max_length",
        truncation=True,
        max_length=512  # או פחות אם את רוצה לחסוך בזיכרון
    )
...
def compute_metrics(pred):
    ...
    probs = pred.predictions[:, 1]  # probability of class 1
...
training_args = TrainingArguments(
    ...
    num_train_epochs=4,  # Updated from 3 to 4
    learning_rate=2e-5,
    weight_decay=0.01,
    logging_dir="./logs",
    logging_steps=100,
